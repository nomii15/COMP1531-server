...
10. For some of the test cases, automated testing was used to ensure that the behaviour was constant for the same variations of the input. This also allows our group to validate further that the functions created are build to meet the needs and wants by Sally and Bob. Some of the test cases which were run automatically were some of the search functions. Theses tests including searching different message in a channel that has no messages and sending a message in a channel and then searching for that message. These types of tests allow our group to validate the function and assure that the function is robust for its intended application.
